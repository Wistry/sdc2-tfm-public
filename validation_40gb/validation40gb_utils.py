"""Utilities for 40 GB extended validation on the 40GB development cube."""
...
def outside_10gb_region(df: pd.DataFrame) -> pd.Series:
    if "x" not in df.columns or "y" not in df.columns:
        raise ValueError("Catalogue must contain x and y columns for 40 GB validation-region filtering.")
