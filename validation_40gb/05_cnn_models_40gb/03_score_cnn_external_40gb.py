#!/usr/bin/env python3
"""Score frozen CNN filtered catalogues for 40 GB extended validation."""
...
OUT_DIR = BASE / "outputs" / "external_cnn_scores"
...
def main() -> None:
    ...
    if not paths:
        raise FileNotFoundError(
            f"No CNN filtered catalogues found in {FILTERED_DIR}. "
            "Run 05_cnn_models_40gb/02_apply_frozen_cnn_external_40gb.py first."
        )
    ...
    summary_path = OUT_DIR / "cnn_external_40gb_score_summary.csv"
    summary.to_csv(summary_path, index=False)
    print(summary[["base_catalog", "cnn_model", "threshold_mode", "dataset_region", "n_candidates", "matches", "false", "score", "status"]].to_string(index=False))
