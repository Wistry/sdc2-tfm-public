#!/usr/bin/env python3
"""Prepare tiled SoFiA runs for 40 GB extended validation."""
...
from validation40gb_utils import (
    CONFIGS,
    CUBE_X_MAX,
    CUBE_X_MIN,
    CUBE_Y_MAX,
...
def write_runner(path: Path, config_run_name: str, tiles: list[dict[str, int | str | bool]]) -> None:
    ...
    for tile in tiles:
        tile_name = str(tile["tile_name"])
        lines.append(f'bash "{VALIDATION_ROOT / "sofia_tile_runs" / config_run_name / tile_name / "run_sofia.sh"}"')
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    path.chmod(0o755)
...
def main() -> None:
    tiles = build_tiles()
    meta_dir = ensure_dir(VALIDATION_ROOT / "outputs" / "tile_metadata")
    ...
    runner_dir = VALIDATION_ROOT / "02_sofia_tile_generation"
