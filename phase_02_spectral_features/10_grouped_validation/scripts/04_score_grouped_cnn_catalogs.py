#!/usr/bin/env python3
"""Officially score grouped-CNN filtered catalogues."""
...
REFS = {
    "phase1_best": 17.524923,
    "phase2_principal": 18.524923,
    "cnn_original": 28.333489815821608,
...
def main() -> None:
    ...
    catalog_paths = sorted(FILTERED.glob("CNN_grouped_*.csv"))
    ...
    if not scores.empty:
        for name, ref in REFS.items():
            scores[f"delta_vs_{name}"] = pd.to_numeric(scores["score"], errors="coerce") - ref
    csv_path = SCORES / "grouped_cnn_official_scores.csv"
