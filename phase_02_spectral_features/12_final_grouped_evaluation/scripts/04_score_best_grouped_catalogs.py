#!/usr/bin/env python3
"""Officially score final grouped filtered catalogues."""
...
def main() -> None:
    ...
    catalog_paths = sorted(FILTERED.glob("*.csv"))
    ...
    for path in catalog_paths:
        ...
        if Sdc2Scorer is None:
            ...
            try:
                ...
                row.update({"status": "OK", "score": score_value_from(score), "matches": attrs.get("n_match"), "false": attrs.get("n_false"), "score_attrs_json": json.dumps(attrs, sort_keys=True)})
    ...
    scores.to_csv(SCORES / "final_grouped_official_scores.csv", index=False)
