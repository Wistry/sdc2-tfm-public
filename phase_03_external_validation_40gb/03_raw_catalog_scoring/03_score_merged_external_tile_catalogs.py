#!/usr/bin/env python3
"""Score merged external tiled catalogues against external 40GB truth."""
...
from phase03_utils import CONFIGS, convert_to_sdc2_submission, ensure_dir, score_submission


TRUTH_EXTERNAL = PHASE3_ROOT / "outputs" / "external_truth" / "sky_ldev_truthcat_v2_external_only.txt"
MERGED_DIR = PHASE3_ROOT / "outputs" / "merged_tile_catalogs"
OUT_DIR = PHASE3_ROOT / "outputs" / "external_tile_scores"


def ensure_external_truth() -> None:
    if TRUTH_EXTERNAL.exists():
        return
    subprocess.run([sys.executable, str(PHASE3_ROOT / "01_region_definition" / "03_filter_truth_external_region.py")], check=True)
